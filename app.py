...
def calculate_tax(income):
    ...
    # Section 87A simplified rule used by this project
...
def income():
    ...
    saved = con.execute(
        "SELECT annual_income FROM income WHERE user_id=?",
        (session["user_id"],)
    ).fetchone()
    ...
    if request.method == "POST":
        ...
        con.execute(
            ...
            (user_id, annual_income)
            VALUES (?,?)
            """,
            (session["user_id"], annual_income)
        )
    ...
    value = saved["annual_income"] if saved else ""
...
def expenses():
    ...
    for i in range(1, 13):
        ...
        if row:
            ...
            <td>₹{row["electricity"]:.0f}</td>

            <td>₹{row["medical"]:.0f}</td>
    ...
    <h2>Saved Monthly Expenditure</h2>
