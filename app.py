...
def init_db():
    ...
    # Old version had UNIQUE(user_id, month).
    # Remove it so the same month can be used in different years.
    cur.execute("""
        ALTER TABLE expenses
        DROP CONSTRAINT IF EXISTS expenses_user_id_month_key
    """)

    # Create correct unique constraint if it doesn't already exist
    cur.execute("""
        DO $$
        BEGIN
            IF NOT EXISTS (
...
def income():
    ...
    if request.method == "POST":
        ...
        monthly_after_tax = (annual_income - tax) / 12
        ...
        <div class="card">
            ...
            <div class="summary">
                ...
                <div class="summary-box">
                    ...
                    <h3>₹{{ "%.2f"|format(monthly_after_tax) }}</h3>
                </div>
            ...
            <div class="analysis">
                ...
                <p><b>87A rebate:</b> Applied in this simplified calculator
                when annual income is up to ₹12,00,000.</p>

                <p><b>Health & Education Cess:</b> 4%</p>
...
def expenses():
    ...
    if request.method == "POST":
        ...
        saved_money = monthly_income - total_expense
    ...
    for row in rows:
        ...
        income_value = float(income_value or 0)
    ...
    if category_totals:
        ...
        highest_expense_value = category_totals[
    ...
    if month_expenses:
        ...
        highest_expense_month_value = month_expenses[
    ...
    if month_savings:
        ...
        highest_saving_value = month_savings[
    ...
    suggestion = "Your spending pattern is recorded for this year."
    ...
    comparison = "Add at least two months to see a monthly comparison."

    if len(rows) >= 2:

        previous_month = rows[-2][0]
        current_month = rows[-1][0]
        ...
        if difference > 0:
            ...
            comparison = (
                f"{current_month} and {previous_month} "
                f"have the same expenditure."
            )
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <h3>{{ selected_year }} - {{ t("expenses") }}</h3>
            ...
            <div class="analysis">
                ...
                <p>
                    <b>{{ t("highest_expense") }}:</b>
                    {{ highest_expense_category }}
                    — ₹{{ "%.2f"|format(highest_expense_value) }}
                </p>

                <p>
                    <b>{{ t("highest_month") }}:</b>
                    {{ highest_expense_month }}
                    — ₹{{ "%.2f"|format(highest_expense_month_value) }}
                </p>

                <p>
                    <b>{{ t("highest_saving_month") }}:</b>
                    {{ highest_saving_month }}
                    — ₹{{ "%.2f"|format(highest_saving_value) }}
                </p>

                <p>
                    <b>{{ t("expense_percent") }}:</b>
                    {{ "%.2f"|format(expense_percentage) }}%
                </p>

                <p>
                    <b>{{ t("saving_percent") }}:</b>
                    {{ "%.2f"|format(saving_percentage) }}%
                </p>
